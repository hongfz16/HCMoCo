...
num_samples=400
tag=ntucocorgbd2s_hrnet_w18_second_stage
ngpu=4
pretrain=./output/cmc_ntumpiirgbd2s_model/CMCRGBD2S_HRNet_RGBD2S_Jig_False_bank_aug_C_linear_0.07_ntucocorgbd2s_official_hrnet_w18_norm_depth_cosine/ckpt_epoch_100.pth
srun -p dsta \
    --job-name=ntucocorgbd2s \
    --gres=gpu:${ngpu} \
    --ntasks=${ngpu} \
    ...
    python -u main_contrast.py \
        ...
        --modality_missing 1 \
        --coco_root data/coco/ \
        --skeleton_meta_name coco_reduce \
        --pretrain ${pretrain} \
